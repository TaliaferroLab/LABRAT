...
def getrowdict(index, row, samp_conds, fraca, fracb):
	...
	conds = []
	for cond in samp_conds:
		conds += [cond] * sum(len(libprep) for libprep in samp_conds.items())
	condas = []
	condbs = []
	for cond in conds:
		if cond == fraca:
			...
			condas.append(0)
			condbs.append(1)
	d['cond1'] = condas #e.g. [1, 1, 1, 1, 0, 0, 0, 0]
	d['cond2'] = condbs #e.g. [0, 0, 0, 0, 1, 1, 1, 1]
...
def getdpsis(psifile):
	...
	#Store relationships of conditions and the samples in that condition
	#It's important that this dictionary be ordered because we are going to be iterating through it
	fracs = ['ML-DmD17-c3_cytosol', 'ML-DmD17-c3_membrane', 'ML-DmD17-c3_insoluble', 'ML-DmD17-c3_total']

	for combination in itertools.combinations(fracs, 2):
		fraca = combination[0]
		fracb = combination[1]
		pvalues = []
		...
		for index, row in psidf.iterrows():
			...
			#If any psi value is NA, the pvalue will be NA
			p = None
			values = d['value']
			if True in np.isnan(values):
				p = np.nan
			...
			if not p:
				with warnings.catch_warnings():
					...
					try:
						#actual model
						md = smf.mixedlm('value ~ cond1 + polyA', data = rowdf, groups = 'cond1', missing = 'drop')
						mdf = md.fit(reml = False) #REML needs to be false in order to use log-likelihood for pvalue calculation

						#null model
						nullmd = smf.mixedlm('value ~ 1 + polyA', data = rowdf, groups = 'samples', missing = 'drop')
						nullmdf = nullmd.fit(reml = False)
		...
		#Add deltapsis, pvalues, and FDRs to df
		deltapsis = list(deltapsidict.values())
		dpsicolname = fraca + '_vs_' + fracb + '_dpsi'
		pvalcolname = fraca + '_vs_' + fracb + '_pval'
		fdrcolname = fraca + '_vs_' + fracb + '_fdr'
		psidf[dpsicolname] = deltapsis
		psidf[pvalcolname] = pvalues
		psidf[fdrcolname] = fdrswithnas
		#psidf = psidf.assign(deltapsi = deltapsis)
		#psidf = psidf.assign(pval = pvalues)
		#psidf = psidf.assign(FDR = fdrswithnas)
